...
namespace GUI_Class
{
    ...
    public partial class SquareControl : PictureBox {
        ...
        protected override void OnPaint(PaintEventArgs e) {
            ...
            var textWidth = textFont.Size * name.Length;
            float textHeight = textFont.Height;
            var textX = e.ClipRectangle.Right - textWidth;
            var textY = e.ClipRectangle.Bottom - textHeight;
            var drawRect = new RectangleF(textX, textY, textWidth, textHeight);
           ...
           const int PLAYER_TOKEN_SIZE = 20;  // pixels.
            ...
            for (var i = 0; i < containsPlayers.Length; i++) {
                if (containsPlayers[i]) {
                    var xPosition = i % PLAYER_TOKENS_PER_ROW;
                    var yPosition = i / PLAYER_TOKENS_PER_ROW;
                    var xPixels = xPosition * (PLAYER_TOKEN_SIZE + PLAYER_TOKEN_SPACING);
                    var yPixels = yPosition * (PLAYER_TOKEN_SIZE + PLAYER_TOKEN_SPACING);
                    var playerTokenColour = players[i].PlayerTokenColour;                                  //UNCOMMENT :Remove  line comments on this and next line whne instructed to do so in Part B specs
