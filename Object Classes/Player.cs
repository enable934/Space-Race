    ...
    public class Player
    {
        ...
        public Brush PlayerTokenColour
        {
            ...
            set
            {
                ...
                using (var g = Graphics.FromImage(PlayerTokenImage))
                {
        ...
        public void Play(Die d1, Die d2)
        {
            ...
            {
                var thisNumber = Position;
                var nextNumber = d1.Roll() + d2.Roll();
                if (Location.NextSquare() == null)
                {
                    Location.LandOn(this);
                    if (thisNumber + nextNumber < Board.Squares.Length-1)
                    {
                        Location = Board.Squares[thisNumber + nextNumber];
                        Position = Board.Squares[thisNumber + nextNumber].Number;
                    }
                    else
                    {
                        var number = Board.Squares.Length - 1 -( nextNumber - (Board.Squares.Length - thisNumber));
                        if (number == 56) number = 54;
                        Location = Board.Squares[number];
                        Position = Board.Squares[number].Number;
                    }
        ...
        private bool ReachedFinalSquare()
        {
            ...
            if (Location == Board.Squares[Board.FinishSquareNumber]) return true;
